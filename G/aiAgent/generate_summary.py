...
PLAIN_MAP: dict[str, str] = {
    ...
    "coreyhaines31/marketingskills": "给 Claude Code/AI Agent 用的营销技能包：转化、文案、SEO、增长工程一锅端",
    "virgiliojr94/book-to-skill": "把技术书 PDF 变成 Claude Code Skill：边干活边查阅、学习、引用",
    "moeru-ai/airi": "可自托管的虚拟伴侣/数字生命：实时语音、能玩 Minecraft/Factorio，多端可用",
    "OpenHands/OpenHands": "开源 AI 驱动开发助手：让智能体帮你写代码、改项目、推进开发",
    "different-ai/openwork": "开源版 Claude Cowork 替代：基于 opencode 的协作式 AI 工作台",
    "AgriciDaniel/claude-seo": "Claude Code 全能 SEO Skill：技术 SEO/GEO、结构化数据、外链与报表等",
    "ahujasid/blender-mcp": "开源 MCP：让任意大模型通过协议操控 Blender 做三维创作",
    "rowboatlabs/rowboat": "开源 AI 同事：带记忆，能帮你持续协作干活",
    "dani-garcia/vaultwarden": "兼容 Bitwarden 的轻量自托管密码库服务（Rust，原 bitwarden_rs）",
    "termux/termux-app": "安卓上的终端模拟器：可扩展一堆 Linux 包，手机里也能敲命令",
    "prisma/prisma": "Node/TypeScript 新一代 ORM：类型安全地访问 Postgres/MySQL/Mongo 等",
    "paperless-ngx/paperless-ngx": "社区强化的文档管理系统：扫描、索引、归档，纸质材料数字化",
    "mrexodia/ida-pro-mcp": "给 IDA Pro 接大模型的 MCP 逆向助手：AI 帮你看反汇编",
    "NdoleStudio/httpsms": "用安卓手机当短信网关：通过简单 HTTP API 收发短信",
    ...
    "TencentCloud/TencentDB-Agent-Memory": "腾讯云 Agent 团队级记忆中枢：对话/文档/代码变成可共享记忆资产",
    "donnemartin/system-design-primer": "大规模系统设计入门宝典，面试系统设计题常备资料",
    "rclone/rclone": "云存储界的 rsync：多云盘同步、挂载与传输命令行工具",
    "zhaoxuya520/reverse-skill": "逆向/授权渗透技能路由包：按需拉起工具链，还能自进化知识库",
    "microsoft/AI-For-Beginners": "微软开源「人人可学 AI」：12 周 24 课从零入门人工智能",
    "home-assistant/core": "本地优先的开源智能家居平台：隐私可控，自己管家里的设备",
    "astral-sh/uv": "Rust 写的超快 Python 包/项目管理器，装依赖快到飞起",
    "livekit/agents": "做实时语音 AI Agent 的框架：对话能听能说还能看",
    "ltaoo/wx_channels_download": "微信视频号下载器：把视频号内容落到本地",
}

CATEGORY_RULES: list[tuple[str, list[str]]] = [
    ("AI Agent / 智能体", ["agent", "智能体", "orchestrat", "multiplex", "fleet", "ade for", "vibe-trading", "hiring-agent", "openinterpreter", "crush", "gnhf", "comet", "agentscope", "speech-to-speech", "agent-native", "agentgateway", "omnigraph", "skillhub", "agent-toolkit", "pentagi", "storm", "awesome-llm-apps", "ai-job-search", "job application", "deeptutor", "kimi-code", "kimi-cli", "k-skill", "agentos", "text-to-cad", "earendil-works/pi", "openhands", "rowboat", "airi", "openwork", "marketingskills", "book-to-skill", "livekit/agents", "voice ai"]),
    ...
    ("爬虫 / 数据采集", ["scrape", "crawl", "firecrawl", "spider", "采集", "mediacrawler", "yt-dlp", "google-maps-scraper", "ghost-downloader", "omniget", "crawl4ai", "wx_channels", "视频号"]),
    ("安全 / 渗透测试", ["secur", "pentest", "vulnerab", "hack", "渗透", "strix", "nuclei", "skillspector", "cybersecurity", "debloat", "pentagi", "ghidra", "ida-pro", "reverse engineering", "reverse-skill"]),
    ("视频 / 多媒体 / 会议", ["video", "meeting", "iptv", "transcri", "montage", "media", "会议", "直播", "opencut", "voicebox", "romm", "smarttube", "immich", "claude-video", "peertube", "webtorrent", "openwhispr", "bevy", "lingbot-map", "palmier", "video-use", "jellium", "jellyfin", "pocket-tts", "tts", "screenpipe", "blender", "seedance", "filmmaking"]),
    ("网络 / 通信 / 隐私", ["chat", "privacy", "quic", "nat", "messaging", "iroh", "simplex", "隐私", "tailscale", "headscale", "ntfy", "cryptomator", "rustdesk", "frp", "canopy", "stalwart", "schollz/croc", "croc", "bitchat", "netbird", "wireguard", "temp_email", "temp email", "vaultwarden", "bitwarden", "httpsms", "sms", "home-assistant", "home automation"]),
    ("数据库 / 存储", ["database", "vector", "storage", "zvec", "sql", "向量库", "turso", "dbx", "elasticsearch", "qdrant", "chat2db", "pocketbase", "prisma", "orm", "rustfs", "minio", "s3-compatible", "object storage", "rclone", "cloud storage"]),
    ...
    ("大模型 / LLM 基础设施", ["llm", "prompt", "gateway", "token", "model", "openai", "claude", "gpt", "gemini", "inference", "omniroute", "lmcache", "timesfm", "slime", "skillopt", "system_prompts", "grok2api", "caveman", "tokscale", "outlines", "structured output", "ai-for-beginners"]),
    ...
    ("语言 / 框架 / 基础库", ["library", "framework", "logging", "spdlog", "compiler", "runtime", "rust-lang/rust", "cupy", "hello-algo", "javaguide", "golang/go", "catch2", "next.js", "chinese-poetry", "spring-boot", "cs249r", "django", "jj-vcs", "pumpkin-mc", "nodejs/node", "cpython", "system-design-primer", "system design"]),
]
